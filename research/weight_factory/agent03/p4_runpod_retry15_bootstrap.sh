...
AUTH="$AGENT/p4_a1_runpod_manager_authorization_retry15_v1.json"
...
echo "AQLEVON_RETRY15_BOOTSTRAP_START $(date -u +%FT%TZ)"
if [ ! -d "$REPO/.git" ]; then
...
a=json.loads(Path("/workspace/MUS-AI/research/weight_factory/agent03/p4_a1_runpod_manager_authorization_retry15_v1.json").read_text())
assert a["authorization_id"]=="P4-A1-RUNPOD-4090-20260920-15-EU-RO-1-NEW-POD"
assert a["authorization_sha256"]=="b68d652255808177fb92a09fb2fdfbba01bc6cbc2f6d7eada957fbbf0da07928"
...
print("RETRY15_FILE_IDENTITY_PASS")
PY
...
auth=json.loads(Path("p4_a1_runpod_manager_authorization_retry15_v1.json").read_text())
...
print("RETRY15_EXACT_AUTHORIZATION_PASS")
PY
...
python - <<'PY' 2>&1 | tee "$ROOT/aqlevon_p4/P4_A1_seed1701_retry15.log"
import json
from pathlib import Path
import p4_gene1_trainer as c
run=json.loads(Path("p4_a1_seed1701_run_manifest_v1.json").read_text())
...
echo "AQLEVON_RETRY15_TOTAL_SCRIPT_SECONDS=$(( $(date +%s) - START_TS ))"
exit "$RC"
