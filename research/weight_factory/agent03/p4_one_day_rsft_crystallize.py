#!/usr/bin/env python3
"""AQLEVON one-day verifier-guided RSFT crystallization (Candidate C).

Candidate C fixes the Candidate-B epoch-accounting bug and uses only PUBLIC
discovery data for gradients. Before training it mines the frozen base model
with rejection sampling on discovery prompts, keeps verifier-passing model
trajectories as extra positives, and retains one PUBLIC discovery oracle per
task as balanced coverage. PUBLIC shadow remains evaluation-only and is never
used for gradients, target construction, checkpoint selection, or tuning.
...
SEED = 1701
MAX_TRAIN_EXAMPLES = 28
EPOCHS = 3
MINING_N = 8
MAX_ON_POLICY_PER_TASK = 1
LR = 1e-5
CAP = 2048
...
def load_inputs(args):
    ...
    for task in tasks.values():
        idx = task_index(task)
        if idx in (0, 1):
            program = task.get("oracle_program")
            if not isinstance(program, list) or not program:
                raise RuntimeError(f"missing_public_oracle:{task['task_id']}")
            canonical = json.dumps(program, ensure_ascii=False, separators=(",", ":"))
            train.append({
                "task_id": task["task_id"],
                "prompt": task["prompt"],
                "target": canonical,
                "source": "public_discovery_oracle_full_coverage",
                "priority": 0,
                "family": task["family"],
                "task": task,
            })
    ...
    train = sorted(train, key=lambda x: (x["family"], x["task_id"]))
    shadow = sorted(shadow, key=lambda x: x["task_id"])
    if len(train) != 28 or len({x["family"] for x in train}) != 14:
        raise RuntimeError(f"bad_discovery_coverage:{len(train)}:{len({x['family'] for x in train})}")
    if len(shadow) != 28:
...
def run(args):
    ...
    mined, mining_rows = mine_discovery_on_policy(base, tokenizer, w02, train_rows, generation)
    train_rows = build_rsft_rows(train_rows, mined)
    on_policy_examples = sum(1 for x in train_rows if x["source"] == "public_discovery_on_policy_verifier_pass")
    if len([x for x in train_rows if x["source"] == "public_discovery_oracle_full_coverage"]) != 28:
        raise RuntimeError("oracle_coverage_changed")
    print(f"AQLEVON_RSFT_MINING_PASS on_policy={on_policy_examples} total_unique_targets={len(train_rows)}", flush=True)
    ...
    for epoch in range(EPOCHS):
        ...
        for row in epoch_rows:
            ...
            used.append({**row, "epoch":epoch+1})
            print(f"AQLEVON_CRYSTALLIZE_UPDATE={global_step}/{steps} epoch={epoch+1}/{EPOCHS} loss={losses[-1]:.6f} grad_norm={grad:.6f} lr={lr:.9g}", flush=True)
            del ids, labels, logits, loss
    if global_step != steps:
        raise RuntimeError(f"optimizer_update_accounting_mismatch:{global_step}:{steps}")
    ...
    pre_by_id={r["task_id"]: bool(r["passed"]) for r in pre["records"]}
    post_by_id={r["task_id"]: bool(r["passed"]) for r in post["records"]}
    regressions=sorted(tid for tid, ok in pre_by_id.items() if ok and not post_by_id.get(tid, False))
    public_gate=bool(improvement >= 0.10 and gain_tasks >= 3 and not regressions)
    print("AQLEVON_CRYSTALLIZE_SHADOW_POST", json.dumps({k:v for k,v in post.items() if k!="records"}, sort_keys=True), flush=True)
    print(f"AQLEVON_CRYSTALLIZE_PUBLIC_GATE={'PASS' if public_gate else 'FAIL'} improvement={improvement:.6f} gain_tasks={gain_tasks} regressions={len(regressions)}", flush=True)
    ...
    receipt=sealed({
        ...
        "shadow_gain_tasks":gain_tasks,"shadow_improvement":improvement,"shadow_regressions":regressions,"public_shadow_gate_pass":public_gate,
...
def main():
    ...
    expected_min_updates = len(train) * EPOCHS
    if expected_min_updates != 84:
        raise RuntimeError(f"bad_minimum_update_schedule:{expected_min_updates}")
    print(f"AQLEVON_RSFT_PUBLIC_PREFLIGHT_PASS train_verified={len(train)} shadow={len(shadow)} epochs={EPOCHS} minimum_updates={expected_min_updates} probe_decision={probe['decision']}", flush=True)
