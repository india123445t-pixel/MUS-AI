...
echo "PREMODEL_CHECK_RUNTIME_PASS"
...
python - <<'PY'
import ray
ray.init(num_cpus=1, num_gpus=0, ignore_reinit_error=True, log_to_driver=False)
@ray.remote
def probe():
    import os
    import torch
...
out=ray.get(probe.remote())
ray.shutdown()
assert out["user"]=="root", out
assert out["torch"].split("+")[0]=="2.8.0", out
assert str(out["cuda"]).startswith("12.8"), out
assert out["flash_attn"]=="2.8.3", out
assert str(out["verl"]).startswith("/workspace/SDPO/verl/"), out
print("RAY_WORKER_RUNTIME_IMPORT_PASS", out)
PY
