...
def _canonical_shard_bytes(admitted: list[dict[str, Any]]) -> bytes:
    ordered = sorted(admitted, key=lambda r: (r["record_content_sha256"], r["record_id"]))
    return b"".join(canonical_json_bytes(row) + b"\n" for row in ordered)


def _manifest_identity_payload(manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "scheme": MANIFEST_ID_SCHEME,
        "admission_policy_sha256": manifest["admission_policy_sha256"],
        "source_registry_snapshot_sha256": manifest["source_registry_snapshot_sha256"],
        "protected_training_contamination_evidence": manifest["protected_training_contamination_evidence"],
        "admission_gate_code_sha256": manifest["admission_gate_code_sha256"],
        "shard_file_sha256": manifest["shard_file_sha256"],
        "admitted_record_content_digest_sha256": manifest["admitted_record_content_digest_sha256"],
        "decision_log_sha256": manifest["decision_log_sha256"],
        "provenance_license_evidence_bundle_sha256": manifest["provenance_license_evidence_bundle_sha256"],
        "language_audit_receipt_sha256": manifest["language_audit_receipt_sha256"],
...
def build_training_shard_manifest(
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "protected_training_contamination_evidence": {
...
def validate_training_shard_manifest(manifest: Any) -> tuple[bool, str]:
    ...
    required = {
        ...
        "protected_training_contamination_evidence", "admission_gate_code_sha256",
    ...
    contamination = manifest["protected_training_contamination_evidence"]
    if not isinstance(contamination, dict) or set(contamination) != {"manifest_sha256", "receipt_sha256"}:
        return False, "invalid_contamination_evidence"
    if not _valid_sha256(contamination["manifest_sha256"]):
        return False, "invalid_contamination_manifest_sha256"
    receipts = contamination["receipt_sha256"]
