...
def build_tournament_decision(
    *, law: dict[str, Any], stage_binding: dict[str, Any], registration: dict[str, Any],
    ...
    parent_surrogate_recipe_decision: dict[str, Any] | None = None,
) -> dict[str, Any]:
    invalid: list[str] = []
    ...
    if stage == "canonical_27b":
        if not isinstance(parent_surrogate_recipe_decision, dict):
            invalid.append("canonical 27B parent surrogate recipe decision missing")
        else:
            parent_errors = validate_surrogate_recipe_decision(parent_surrogate_recipe_decision, law=law)
            invalid.extend([f"parent surrogate recipe:{x}" for x in parent_errors])
            if parent_surrogate_recipe_decision.get("decision_status") != "SURROGATE_RECIPE_SELECTED":
                invalid.append("canonical 27B parent surrogate recipe is not selected")
            if parent_surrogate_recipe_decision.get("surrogate_recipe_decision_sha256") != stage_binding.get("parent_decision_receipt_sha256"):
                invalid.append("canonical 27B parent surrogate recipe receipt hash mismatch")
            if len(reg_entries) == 1:
                only = next(iter(reg_entries.values()))
                if only.get("candidate_slot") != parent_surrogate_recipe_decision.get("selected_candidate_slot"):
                    invalid.append("canonical 27B candidate slot differs from selected surrogate recipe")
                if only.get("arm_id") != parent_surrogate_recipe_decision.get("selected_arm_id"):
                    invalid.append("canonical 27B arm differs from selected surrogate recipe")
                if only.get("recipe_spec_sha256") != parent_surrogate_recipe_decision.get("selected_recipe_spec_sha256"):
                    invalid.append("canonical 27B recipe differs from selected surrogate recipe")
