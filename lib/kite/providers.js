...
function resolveHealthUrl(endpoint,settings={}){
  const explicit=settings?.self_hosted_health_url||process.env.KITE_MODEL_HEALTH_URL||process.env.LOCAL_MODEL_HEALTH_URL;
...
export function resolveSelfHostedConfig(settings={}){
  const rawEndpoint=settings?.self_hosted_url||settings?.kite_model_url||process.env.KITE_MODEL_URL||process.env.LOCAL_MODEL_URL||null;
  const endpoint=normalizeEndpoint(rawEndpoint);
  const model=String(settings?.self_hosted_model||process.env.KITE_MODEL_NAME||process.env.LOCAL_MODEL_NAME||'KITE-27B');
  const key=process.env.KITE_MODEL_KEY||process.env.LOCAL_MODEL_KEY||'';
...
export async function checkSelfHostedHealth(settings={},opts={}){
  const cfg=resolveSelfHostedConfig(settings);
  if(!cfg.endpoint)return Object.freeze({ok:false,configured:false,provider:'kite-engine',model:cfg.model,status:0,error_class:'ENV_MISSING'});
  ...
  try{
    ...
    return Object.freeze({ok:false,configured:true,provider:'kite-engine',model:cfg.model,status:0,error_class:classifyProviderFailure({error,keyPresent:true})});
  }
}

async function selfHosted(messages,settings,opts={}){
  const provider='kite-engine';
  const cfg=resolveSelfHostedConfig(settings);
...
async function openRouter(messages,webSearch,settings,opts={}){
  ...
  return openAICompatible({
    ...
    temperature:Number(opts.temperature??settings?.temperature??0.4),headers:{'X-Title':'KITE AI'},payloadExtra,
...
export async function generateModelResponse(messages,webSearch,settings={},opts={}){
  ...
  async function record(result){
    if(result?.diagnostic){
      ...
      if(opts.logDiagnostics===true)console.warn('KITE_PROVIDER_DIAGNOSTIC',result.diagnostic);
    }
