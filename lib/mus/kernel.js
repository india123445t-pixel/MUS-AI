...
import { MUS_BOS_VERSION, ROUTE_MODES, VERIFICATION_RESULTS, HIGH_CONSEQUENCE_TERMS } from './constants.js';
...
import { redactSecrets, sanitizeHistory, stableJson } from './security.js';
...
function buildAcceptanceCriteria({domains,flags}){
  const criteria=[
...
export function buildTaskContract(input,opts={}){
  ...
  const domains=detectDomains(redactedInput);
  ...
  if(domains.some(d=>['software','math','data','science','research','operations'].includes(d)))score++;
  ...
  const formalVerificationRequired=routeMode!==ROUTE_MODES.FAST||domains.some(d=>['software','math','data','science','research','operations'].includes(d));
  const contract={
    ...
    success_criteria:buildAcceptanceCriteria({domains,flags}),
...
export function buildRouteDecision(contract,settings={},body={}){
  ...
  const autoSearch=contract.external_evidence_required&&searchConfigured&&paidSearchAllowed;
  const useSearch=searchConfigured&&paidSearchAllowed&&(requestedSearch||autoSearch);
...
export function adjudicateFormalVerification({contract,route,candidate,advisoryVerifier=null,deterministic={}}){
  ...
  // A second model can help choose/repair a response, but does not mint formal VERIFIED by itself.
...
export function chooseCandidate(candidates=[],advisory=null){
  ...
  // No majority vote. Deterministic stable default is first successfully routed candidate.
  return candidates[0];
}
