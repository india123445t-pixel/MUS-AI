...
async function selfHosted(messages,settings,opts={}){
  ...
  const model=settings?.self_hosted_model||process.env.MUS_MODEL_NAME||process.env.LOCAL_MODEL_NAME||'mus-ai';
  ...
  if(!endpoint)return failure({provider,model,route,keyPresent:false});
  try{
