import {computeChildCandidateSha256} from './child-candidate.js';
...
export function evaluateChildTeachingCandidate(candidate){
  ...
  if(candidate?.scope!=='owner-controlled-child-lab')issues.push('SCOPE_INVALID');
  if(candidate?.isolation?.owner_controlled!==true)issues.push('OWNER_POLICY_MISSING');
  ...
  return Object.freeze({
    ...
    verdict,
    training_allowed:candidate?.isolation?.training_lane_write===true,
    gpu_allowed:candidate?.isolation?.gpu_request_allowed===true,
    production_weight_write:candidate?.isolation?.production_weight_write===true,
    training_lane_write:candidate?.isolation?.training_lane_write===true,
    worker03_access:candidate?.isolation?.worker03_access===true,
    automatic_promotion:candidate?.isolation?.automatic_promotion===true,
  });
}
