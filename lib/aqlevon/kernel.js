...
export function buildSystemPrompt({contract,lessons=[],personalization='',memories=[]}){
  ...
  const userPersonalization=redactSecrets(String(personalization||'')).slice(0,4000).trim();
  const userMemories=Array.isArray(memories)?memories.slice(0,16).map(x=>redactSecrets(String(x?.content??x?.text??x??'')).slice(0,1000).trim()).filter(Boolean):[];
  const userContext=[
...
export function buildMessages({input,history,contract,lessons,personalization='',memories=[],maxHistory=16}){
