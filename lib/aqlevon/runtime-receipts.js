...
export const CANONICAL_HASH_PROFILE=P2_HASH_PROFILE;
...
export function canonicalJson(value){
  function encode(item){
    ...
    if(typeof item==='string')return JSON.stringify(normalizeString(item));
    if(typeof item==='number'){
      if(!Number.isFinite(item))throw new TypeError('non_finite_number');
      if(Number.isInteger(item)){
        if(!Number.isSafeInteger(item))throw new TypeError('unsafe_integer');
        return String(Object.is(item,-0)?0:item);
      }
      return JSON.stringify(canonicalDecimalString(item));
    }
    ...
    if(item&&typeof item==='object'){
      ...
      for(const key of keys){
        if(!/^[\x00-\x7f]+$/.test(key))throw new TypeError('non_ascii_object_key');
        if(item[key]===undefined)throw new TypeError('undefined_json_value');
        fields.push(JSON.stringify(key)+':'+encode(item[key]));
      }
      return '{'+fields.join(',')+'}';
    }
    throw new TypeError('unsupported_json_value');
...
function isCanonicalNonNegativeNumeric(value){
  ...
  if(typeof value==='string'&&value.includes('.')&&CANONICAL_DECIMAL_RE.test(value)){
    const n=Number(value);return Number.isFinite(n)&&n>=0&&!Number.isInteger(n)&&canonicalDecimalString(n)===value;
  }
  return false;
}
...
export function buildApprovedTruthAuthority(truth={}){
  ...
  for(const receipt of Array.isArray(evaluationReceipts)?evaluationReceipts:[]){
    const rawId=String(receipt?.receipt_sha256||'');
    const rawCandidate=String(receipt?.candidate_artifact_manifest_sha256||'');
    const id=rawId.toLowerCase();
    const candidate=rawCandidate.toLowerCase();
    const status=receipt?.final_status;
    if(receipt?.schema_version!==1||receipt?.receipt_kind!==EVALUATION_DECISION_RECEIPT_KIND||receipt?.hash_profile!==P2_HASH_PROFILE||!verifyP2SelfDigest(receipt)||!isSha256Hex(rawId)||!isSha256Hex(rawCandidate)||!['INVALID','REJECTED','PROMOTION_ELIGIBLE'].includes(status)){
  ...
  for(const receipt of Array.isArray(objectiveReceipts)?objectiveReceipts:[]){
    const rawId=String(receipt?.receipt_sha256||'');
    const rawAttempt=String(receipt?.attempt_receipt_sha256||'');
    const rawCandidate=String(receipt?.candidate_artifact_manifest_sha256||'');
    const id=rawId.toLowerCase();
    const attempt=rawAttempt.toLowerCase();
    const candidate=rawCandidate.toLowerCase();
    const verdict=receipt?.verdict;
    if(receipt?.schema_version!==1||receipt?.hash_profile!==P2_HASH_PROFILE||!nonemptyId(receipt?.receipt_kind)||!verifyP2SelfDigest(receipt)||!isSha256Hex(rawId)||!isSha256Hex(rawAttempt)||!isSha256Hex(rawCandidate)||!['PASS','FAIL','INVALID'].includes(verdict)){
