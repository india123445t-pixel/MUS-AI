...
export function buildApprovedTruthAuthority(truth={}){
  ...
  for(const receipt of Array.isArray(evaluationReceipts)?evaluationReceipts:[]){
    ...
    if(receipt?.schema_version!==1||(receipt?.receipt_kind??receipt?.manifest_kind)!==EVALUATION_DECISION_RECEIPT_KIND||!isSha256Hex(id)||!isSha256Hex(candidate)||!['INVALID','REJECTED','PROMOTION_ELIGIBLE'].includes(status)){
