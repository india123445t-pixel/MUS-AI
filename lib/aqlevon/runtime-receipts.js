...
function receiptSelfHashMatches(receipt){
  if(!receipt||typeof receipt!=='object'||Array.isArray(receipt)||!isSha256Hex(receipt.receipt_sha256))return false;
  const {receipt_sha256,...body}=receipt;
  return sha256Canonical(body)===receipt_sha256.toLowerCase();
}
