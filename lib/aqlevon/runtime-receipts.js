...
export const CANONICAL_HASH_PROFILE='AQLEVON_CANONICAL_JSON_SHA256_V1';
...
function canonicalDecimalString(value){
  if(!Number.isFinite(value)||Number.isInteger(value))throw new TypeError('non_integral_finite_number_required');
  const negative=value<0;
  let raw=Math.abs(value).toString().toLowerCase();
  if(!raw.includes('e'))return `${negative?'-':''}${raw}`;
  const [mantissa,expText]=raw.split('e');
  const exp=Number(expText);
  const point=mantissa.indexOf('.');
  const digits=mantissa.replace('.','');
  const integerDigits=(point===-1?mantissa.length:point)+exp;
  let plain;
  ...
  if(!plain.includes('.'))throw new TypeError('non_integral_number_canonicalized_to_integer');
...
export function isCanonicalDecimalString(value){return typeof value==='string'&&CANONICAL_DECIMAL_RE.test(value)&&value!=='-0'}
function receiptSelfHashMatches(receipt){
  if(!receipt||typeof receipt!=='object'||Array.isArray(receipt)||receipt.hash_profile!==CANONICAL_HASH_PROFILE||!isSha256Hex(receipt.receipt_sha256))return false;
  const {receipt_sha256,...body}=receipt;
  return sha256Canonical(body)===receipt.receipt_sha256.toLowerCase();
}
...
function finiteNonNegativeOrNull(value){
  ...
  if(isCanonicalDecimalString(value))return Number(value)>=0&&Number.isFinite(Number(value));
  return false;
}
function integerNonNegativeOrNull(value){return value===null||(Number.isSafeInteger(value)&&value>=0)}
...
export function verifyRuntimeAttemptReceipt(receipt){
  ...
  if(receipt?.hash_profile!==CANONICAL_HASH_PROFILE)reasons.push('hash_profile');
  ...
  if(reasons.length===0&&!receiptSelfHashMatches(receipt))reasons.push('receipt_self_hash_mismatch');
...
export function buildApprovedTruthAuthority(truth={}){
  ...
  for(const receipt of Array.isArray(evaluationReceipts)?evaluationReceipts:[]){
    ...
    if(receipt?.schema_version!==1||receipt?.receipt_kind!==EVALUATION_DECISION_RECEIPT_KIND||receipt?.hash_profile!==CANONICAL_HASH_PROFILE||!isSha256Hex(id)||!isSha256Hex(candidate)||!['INVALID','REJECTED','PROMOTION_ELIGIBLE'].includes(status)||!receiptSelfHashMatches(receipt)){
  ...
  for(const receipt of Array.isArray(objectiveReceipts)?objectiveReceipts:[]){
    ...
    if(receipt?.hash_profile!==CANONICAL_HASH_PROFILE||!isSha256Hex(id)||!isSha256Hex(attempt)||!isSha256Hex(candidate)||!['PASS','FAIL','INVALID'].includes(verdict)||!receiptSelfHashMatches(receipt)){
...
export function resolveAttemptVerification(attemptReceipt,authority){
  ...
  if(evaluations.length){
    ...
    return {verified:false,source:'evaluation_decision',reason:'candidate_level_not_attempt_authority',conflict:false};
  }
  return {verified:false,source:null,reason:'no_approved_per_attempt_truth_receipt',conflict:false};
}
