...
const PROFILE_ROWS={
  ...
  'p4-surrogate-1x24':{
    purpose:'P4 Gene #1 surrogate method tournament on low-cost 24GB-class GPU',gpu_count:1,min_vram_mib_each:22000,
    planned_vram_ceiling_mib_each:26000,launcher:'direct',precision_policy:'surrogate_adapter_training',
  },
  'p4-gene1-27b-1x80':{
    purpose:'P4 canonical Qwen3.8-27B Gene #1 adapter training',gpu_count:1,min_vram_mib_each:78000,
    planned_vram_ceiling_mib_each:null,launcher:'direct',precision_policy:'bf16_lora_only',
  },
...
const RECEIPT_ORIGINS=new Set(['owned','donated','free','paid_manager_authorized']);
...
export function validateExecutionAuthorization({authorized,computeOrigin}={}){
  ...
  if(!ZERO_CASH_ORIGINS.has(origin))reasons.push('compute_origin_not_zero_cash');
  return Object.freeze({ok:reasons.length===0,compute_origin:ZERO_CASH_ORIGINS.has(origin)?origin:null,reasons:Object.freeze(reasons)});
}
...
export function buildComputeAttemptReceipt({
  ...
  if(!RECEIPT_ORIGINS.has(String(computeOrigin)))throw new TypeError('compute_origin');
...
export function verifyComputeAttemptReceipt(receipt){
  ...
  if(!RECEIPT_ORIGINS.has(receipt?.compute_origin))reasons.push('compute_origin');
