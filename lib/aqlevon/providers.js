...
export function resolveSelfHostedConfig(settings={}){
  const rawEndpoint=process.env.AQLEVON_MODEL_URL||'https://api.runpod.ai/v2/tawpsv5oxepmo4/runsync';
  const endpoint=normalizeEndpoint(rawEndpoint);
  const model=String(process.env.AQLEVON_MODEL_NAME||'AQLEVON-4B-Auth16');
  const key=process.env.AQLEVON_MODEL_KEY||process.env.RUNPOD_API_KEY||'';
