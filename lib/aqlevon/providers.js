...
export async function checkSelfHostedHealth(settings={},opts={}){
  ...
  if(cfg.runpod_endpoint_id){
    if(!cfg.runpod_key)return Object.freeze({ok:false,configured:true,provider:'aqlevon-engine',model:cfg.model,status:0,error_class:'ENV_MISSING',transport:'runpod-serverless'});
    try{
      const response=await fetchImpl(`https://api.runpod.ai/v2/${cfg.runpod_endpoint_id}/health`,{
        method:'GET',
      ...
      return Object.freeze({ok:false,configured:true,provider:'aqlevon-engine',model:cfg.model,status:0,error_class:classifyProviderFailure({error,keyPresent:true}),transport:'runpod-serverless'});
    }
  }
  try{
