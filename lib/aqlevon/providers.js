...
async function openRouter(messages,webSearch,settings,opts={}){
  ...
  return openAICompatible({
    ...
    temperature:Number(opts.temperature??settings?.temperature??0.4),headers:{'X-Title':'AQLEVON AI'},payloadExtra,
...
export async function generateModelResponse(messages,webSearch,settings={},opts={}){
  ...
  const mode=settings?.runtime_mode||'self_hosted_only';
  const diagnostics=[];
  const external=[
  ...
  async function record(result){
    if(result?.diagnostic){
      ...
      if(opts.logDiagnostics===true)console.warn('AQLEVON_PROVIDER_DIAGNOSTIC',result.diagnostic);
    }
