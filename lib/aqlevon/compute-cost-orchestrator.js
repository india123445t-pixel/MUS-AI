...
const WORK_KEYS=new Set(['gpu_count','gpu_seconds','peak_vram_bytes','tokens_processed','tokens_generated','examples_processed','retry_index','outcome']);
const SUMMARY_KEYS=new Set([
  ...
  'attempts','gpu_seconds_total','billed_wall_seconds_total','tokens_processed_total','examples_processed_total',
...
export function validateP4ExecutionAuthorization({
  ...
  else if(check.ok&&rate>Number(managerAuthorization.max_hourly_rate_usd))reasons.push('provider_hourly_rate_exceeds_authorization');
...
export function buildComputeCostReceipt({
  ...
  if(!integer(peakVramBytes,{min:1}))throw new TypeError('peak_vram_bytes');
  if(!integer(tokensProcessed)||!integer(tokensGenerated)||!integer(examplesProcessed))throw new TypeError('work_counters');
  ...
  const draft={
    ...
    work:{
      ...
      examples_processed:examplesProcessed,
...
export function verifyComputeCostReceipt(receipt){
  ...
  if(!exactKeys(receipt?.work,WORK_KEYS))reasons.push('work_schema');
    ...
    for(const key of ['peak_vram_bytes','tokens_processed','tokens_generated','examples_processed','retry_index'])if(!integer(receipt.work[key]))reasons.push(`work_${key}`);
...
export function buildVerifiedCandidateCostSummary({
  ...
  let gpu=0,billed=0,tokens=0,examples=0,egress=0,estimated=0,actual=0,actualComplete=true;
  for(const r of costReceipts){
    ...
    tokens+=r.work.tokens_processed;
    examples+=r.work.examples_processed;
...
export function verifyVerifiedCandidateCostSummary(summary){
  ...
  for(const key of ['attempts','tokens_processed_total','examples_processed_total','artifact_egress_bytes_total'])if(!integer(summary?.[key]))reasons.push(key);
