...
export function validateRuntimeAttemptReceipt(receipt){
  ...
  const allowedTop=['schema_version','receipt_kind','attempt_id','task_id','candidate_artifact_manifest_sha256','request_harness_identity','transport_outcome','runtime_metrics','runtime_metric_provenance','raw_result_sha256','receipt_sha256'];
  ...
  if(receipt.receipt_kind!==RUNTIME_ATTEMPT_RECEIPT_KIND)errors.push('receipt_kind');
...
function validateEvaluationReceiptIdentity(receipt){
  ...
  return receipt.receipt_kind===EVALUATION_DECISION_RECEIPT_KIND
...
export function resolveAttemptVerification(attemptReceipt,{
  ...
  verificationJoins=[],
  managerApprovedVerificationReceiptSha256=[],
}={}){
  ...
  const managerApproved=new Set((Array.isArray(managerApprovedVerificationReceiptSha256)?managerApprovedVerificationReceiptSha256:[])
    .filter(isSha256).map(value=>String(value).toLowerCase()));
  ...
  for(const join of Array.isArray(verificationJoins)?verificationJoins:[]){
    ...
    if(!managerApproved.has(verificationHash))continue;
