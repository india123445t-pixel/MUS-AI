import {CANONICAL_HASH_PROFILE,buildApprovedTruthAuthority,isSha256Hex,resolveAttemptVerification,sha256Canonical,verifyRuntimeAttemptReceipt} from './runtime-receipts.js';

export const RUNTIME_EFFICIENCY_SCOPE_KIND='AQLEVON_RUNTIME_OPERATIONAL_EFFICIENCY_SCOPE_V1';
...
function buildPopulationScope(attemptReceipts=[]){
  const ids=attemptReceipts.map(receipt=>isSha256Hex(receipt?.receipt_sha256)?String(receipt.receipt_sha256).toLowerCase():null);
  const bound=ids.every(Boolean);
  if(!bound){
    return Object.freeze({
      scope_kind:RUNTIME_EFFICIENCY_SCOPE_KIND,
      hash_profile:CANONICAL_HASH_PROFILE,
      identity_status:'UNAVAILABLE_INVALID_INPUT',
      attempt_count:attemptReceipts.length,
      attempt_receipt_sha256s:null,
      population_identity_sha256:null,
    });
  }
  const body={
    scope_kind:RUNTIME_EFFICIENCY_SCOPE_KIND,
    ...
    attempt_receipt_sha256s:Object.freeze([...ids].sort()),
  };
  return Object.freeze({...body,identity_status:'BOUND',population_identity_sha256:sha256Canonical(body)});
}

function invalidEfficiencySummary(attemptReceipts,reasons){
  const attempts=Array.isArray(attemptReceipts)?attemptReceipts.length:0;
  return Object.freeze({
    schema:'aqlevon-verified-efficiency-summary-v3',
    status:'INVALID',
    truth_policy:'per_attempt_receipt_join_only',
    population_scope:buildPopulationScope(Array.isArray(attemptReceipts)?attemptReceipts:[]),
    ...
    approved_evaluation_receipts:null,
    approved_objective_receipts:null,
...
export function summarizeVerifiedEfficiency(attemptReceipts=[],truth={}){
  if(!Array.isArray(attemptReceipts))return invalidEfficiencySummary([],['attempt_receipts_type']);
  const invalid=[];
  ...
  if(invalid.length)return invalidEfficiencySummary(attemptReceipts,invalid);
  ...
  return Object.freeze({
    ...
    status:'VALID',
    truth_policy:'per_attempt_receipt_join_only',
    population_scope:buildPopulationScope(attemptReceipts),
    ...
    approved_evaluation_receipts:authority.approvedEvaluationCount,
    approved_objective_receipts:authority.approvedObjectiveCount,
