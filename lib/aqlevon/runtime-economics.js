...
export function summarizeVerifiedEfficiency(records=[]){
  ...
  for(const record of Array.isArray(records)?records:[]){
    if(record?.verified===true)verifiedSuccesses++;
    const metrics=record?.runtime_metrics||record?.metrics||record;
  ...
  const attempts=Array.isArray(records)?records.length:0;
  return Object.freeze({
    attempts,
