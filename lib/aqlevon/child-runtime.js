...
export function resolveChildRuntime(){
  ...
  const runpodKey=String(process.env.AQLEVON_CHILD_RUNPOD_KEY||process.env.AQLEVON_CHILD_MODEL_KEY||'').trim();
...
export async function childHealth(opts={}){
  ...
  if(cfg.runpod_endpoint_id){
    if(!cfg.runpod_key)return Object.freeze({ok:false,configured:true,status:0,error_class:'ENV_MISSING',model:cfg.model,protocol:cfg.protocol,transport:'runpod-serverless'});
    try{
      const r=await fetchImpl(`https://api.runpod.ai/v2/${cfg.runpod_endpoint_id}/health`,{
        method:'GET',
        headers:{Authorization:`Bearer ${cfg.runpod_key}`},
        cache:'no-store',
        signal:AbortSignal.timeout(Number(opts.timeoutMs||5000))
      });
      return Object.freeze({ok:r.ok,configured:true,status:r.status,error_class:r.ok?null:(r.status===401||r.status===403?'AUTH_ERROR':'RUNTIME_UNHEALTHY'),model:cfg.model,protocol:cfg.protocol,transport:'runpod-serverless'});
    }catch{
      return Object.freeze({ok:false,configured:true,status:0,error_class:'NETWORK_ERROR',model:cfg.model,protocol:cfg.protocol,transport:'runpod-serverless'});
    }
  }
...
export async function generateChildResponse({messages=[],persona='',lessons=[],memories=[],trial=null,ownerPolicy=null,temperature=0.5}={}){
  const cfg=resolveChildRuntime();
  ...
  const system=[
    'You are AQLEVON Child Lab, an experimental personality controlled by the authenticated project owner.',
    'Operational capabilities are controlled by OWNER_POLICY below. Do not invent capabilities that are disabled or not connected.',
    'Do not claim an external effect happened unless an actual connected tool/runtime returned evidence of that effect.',
    'OWNER_POLICY:\n'+JSON.stringify(policy),
    'Follow the owner-defined personality and lessons for this lab.',
