...
export function resolveChildRuntime(){
  const runpodEndpointId=String(process.env.AQLEVON_CHILD_RUNPOD_ENDPOINT_ID||process.env.AQLEVON_RUNPOD_ENDPOINT_ID||'').trim();
  const runpodKey=String(process.env.RUNPOD_API_KEY||process.env.AQLEVON_CHILD_MODEL_KEY||'').trim();
