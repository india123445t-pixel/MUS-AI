...
const DEFAULT_PLUGINS = [
  { id:'web-search', name:'Web Search', description:'Search the web when AQLEVON runtime supports it.', installed:1, enabled:0, availableScopes:['READ'], connection:{status:'connected',scopes:['READ']} },
  { id:'code-interpreter', name:'Code Interpreter', description:'Local/browser coding tools.', installed:1, enabled:1, availableScopes:['READ'], connection:{status:'connected',scopes:['READ']} },
  { id:'image-gen', name:'Image Generation', description:'Image generation tool surface.', installed:1, enabled:1, availableScopes:['CREATE'], connection:{status:'connected',scopes:['CREATE']} },
...
function blankState(){
  return {
    ...
    settings:{
      theme:'dark', personalization:'', voice_enabled:'', notifications:'', language:'en',
...
function load(){
  ...
  try{
    ...
    return {...blankState(),...x,settings:{...blankState().settings,...(x.settings||{})},plugins:Array.isArray(x.plugins)&&x.plugins.length?x.plugins:DEFAULT_PLUGINS};
...
async function get(path){
  ...
  if(p==='/bootstrap') return {mode:'public-browser',workspace:{id:'local',name:'AQLEVON Workspace',created_at:now()},aiConfigured:true,runtime:{provider:'AQLEVON',browserStorage:true}};
  if(p==='/chats'){
  ...
  if(p==='/scheduled') return s.scheduled.slice().sort((a,b)=>String(b.created_at).localeCompare(String(a.created_at))).map(x=>({...x,next_run:x.next_run||calcNext(x),runs:x.runs||[]}));
...
async function post(path, body={}){
  ...
  if(p==='/scheduled'){
    const item={id:uid(),title:body.title||'Automation',kind:body.kind||'once',prompt:body.prompt||'',runAt:body.runAt||'',interval_minutes:Number(body.intervalMinutes||60),condition_query:body.conditionQuery||'',enabled:true,created_at:now(),runs:[]};item.next_run=calcNext(item);update(s=>{s.scheduled.unshift(item);return s});return item;
  }
  ...
  m=p.match(/^\/dev\/repos\/([^/]+)\/(commit|checkpoints|rollback|run)$/);if(m){
    ...
    update(s=>{const r=s.repos[name];if(!r)throw Error('repository not found');
      ...
      if(action==='run'){const cmd=String(body.command||'').trim();let stdout='';if(cmd==='ls'||cmd==='dir')stdout=Object.keys(r.files||{}).join('\n');else if(cmd.startsWith('cat '))stdout=r.files[cmd.slice(4).trim()]||'';else stdout='Command execution is disabled in the public browser sandbox. Use the downloadable AQLEVON runtime for real shell commands.';out={stdout,stderr:'',exitCode:0}}
      return s}); return out;
  }
  throw Error('Not found: '+path);
}
...
async function patch(path, body={}){
  ...
  m=p.match(/^\/scheduled\/([^/]+)$/);if(m){update(s=>{const x=s.scheduled.find(v=>v.id===decodeURIComponent(m[1]));if(!x)throw Error('not found');Object.assign(x,body);x.next_run=calcNext(x);return s});return out}
