...
export default function AdminPage(){
  ...
  const [ownerMessages,setOwnerMessages]=useالحالة([{role:'assistant',text:'أنا AQLEVON نواة المالك. أعمل داخل لوحة المالك الخاصة. أستطيع تحليل المشروع وتحضير المهمة قابلة للموافقة، ولا أدّعي تنفيذ أي إجراء خارجي من دون التصريح/الإيصال فعلي.'}]);
  ...
  const executorالحالة=hasInFlight?'LIVE':attempts.length?'خامل':'غير متصل';
    ...
    <main className="owner-admin-main">
      ...
      {tab==='owner'&&<>
        ...
        <section className="command-system-strip">
          ...
          <button onClick={()=>setTab('infrastructure')}><span>المنفّذون</span><b>{executorالحالة}</b><small>موصلات Git والمتصفح والنشر</small></button>
      ...
      {tab==='traces'&&<>
        <section className="trace-toolbar">
          ...
          <div className="trace-metrics"><span>P50 <b>{p50Latency?p50Latency+'ms':'—'}</b></span><span>P95 <b>{p95Latency?p95Latency+'ms':'—'}</b></span><span>P99 <b>{p99Latency?p99Latency+'ms':'—'}</b></span><span>نجاح الأدوات <b>{toolنجاحRate}%</b></span><span>المحاولات <b>{attempts.length}</b></span><span>الإيصالات <b>{receipts.length}</b></span></div>
        ...
        <section className="trace-workbench">
          ...
          <div className="trace-tree-pane">
            ...
            {نشطةالمحاولة?<div className="trace-tree">
              <div className="trace-node root"><i/><div><span>مهمة</span><b>{نشطةالمحاولةTask?.title||'AQLEVON المهمة'}</b><small>{نشطةالمحاولة.task_id}</small></div></div>
              <div className="trace-node"><i/><div><span>نية تنفيذ</span><b>{نشطةالمحاولةالنية التنفيذية?.semantic_action||'Action intent'}</b><small>{نشطةالمحاولةالنية التنفيذية?.canonical_resource||'No canonical resource'}</small></div></div>
              <div className="trace-node نشطة"><i/><div><span>محاولة</span><b>{نشطةالمحاولة.phase} · {نشطةالمحاولة.outcome}</b><small>{نشطةالمحاولة.provider_operation_id||'No provider operation id'}</small></div></div>
              <div className={`trace-node ${نشطةالمحاولةالإيصال?'verified':'muted'}`}><i/><div><span>إيصال</span><b>{نشطةالمحاولةالإيصال?.executor_reported_outcome||'No receipt yet'}</b><small>{نشطةالمحاولةالإيصال?.executor_identity||'الأدلة pending'}</small></div></div>
              {نشطةالمحاولةالتدقيق.slice(0,5).map(e=><div className="trace-node audit" key={e.id}><i/><div><span>تدقيق</span><b>{e.event_type}</b><small>{when(e.created_at)}</small></div></div>)}
          ...
          <aside className="trace-inspector">
            ...
            {نشطةالمحاولة&&<>
              ...
              <pre className="trace-json">{JSON.stringify(نشطةالمحاولةالإيصال?{executor:نشطةالمحاولةالإيصال.executor_identity,transport:نشطةالمحاولةالإيصال.transport_status,outcome:نشطةالمحاولةالإيصال.executor_reported_outcome,operation:نشطةالمحاولةالإيصال.provider_operation_id}:{status:'NO_إيصال'},null,2)}</pre>
            </>}
          </aside>
        </section>
        <section className="model-trace-strip">
          <div className="trace-pane-title"><b>حركة النموذج</b><span>{logs.length} تتبّعات حديثة</span></div>
      ...
      {tab==='brain'&&<>
        ...
        <section className="panel-grid">
          <div className="admin-panel"><div className="panel-head"><div><span className="eyebrow">القرارات الأخيرة</span><h3>ذاكرة التدقيق</h3></div></div><div className="owner-timeline">{audit.slice(0,18).map(e=><div key={e.id}><i/><div><b>{e.event_type}</b><span>{e.subject_type||'النظام'} · {when(e.created_at)}</span></div></div>)}</div></div>
      ...
      {tab==='model_lab'&&<>
        <section className="model-lab-hero">
          <div><span className="eyebrow">مختبر النموذج</span><h2>{short(latestالنموذج,34)}</h2><p>قياس الجودة والمهارات والتتبّعات وحالة التعلم قبل أي ادعاء بتحسن النموذج.</p></div>
      ...
      {tab==='infrastructure'&&<>
        ...
        <section className="infra-matrix">
          ...
          <div className="infra-row"><div><i className={status?.self_hosted_configured?'ok':'warn'}/><b>AQLEVON ذاتي الاستضافة</b></div><span>{status?.self_hosted_configured?'متصل':'غير متصل'}</span><small>{settings?.runtime_mode||'runtime mode غير محسومة'}</small></div>
      ...
      {tab==='security'&&<>
        ...
        <section className="security-layout">
          <div className="security-policy">
            ...
            <div><span>التنفيذ</span><b>{executorالحالة}</b><small>لا يعتبر منفّذ الأمن متصلًا من دون موصل تنفيذ حقيقي.</small></div>
            <div><span>الأدلة</span><b>{receipts.length} إيصالS</b><small>أي ادعاء تنفيذ يتطلب أدلة مسجلة.</small></div>
