...
const CHAT_KEY='kite-ai-public-chats-v2';
const SESSION_KEY='kite-ai-public-session-v1';
...
function Icon({name,size=18}){
  ...
  const paths={
    plus:<><path d="M12 5v14M5 12h14"/></>,chat:<><path d="M21 15a4 4 0 0 1-4 4H8l-5 3 1.7-5A7 7 0 0 1 3 12V8a5 5 0 0 1 5-5h9a4 4 0 0 1 4 4z"/></>,send:<><path d="m22 2-7 20-4-9-9-4z"/><path d="M22 2 11 13"/></>,globe:<><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a14 14 0 0 1 0 18M12 3a14 14 0 0 0 0 18"/></>,menu:<><path d="M4 7h16M4 12h16M4 17h16"/></>,install:<><path d="M12 3v12M7 10l5 5 5-5"/><path d="M5 21h14"/></>,thumbUp:<><path d="M7 10v10H3V10h4Zm0 10h9a3 3 0 0 0 3-2.4l1-5A3 3 0 0 0 17 9h-4l1-4a2 2 0 0 0-3.6-1.5L7 10"/></>,thumbDown:<><path d="M7 14V4H3v10h4Zm0-10h9a3 3 0 0 1 3 2.4l1 5A3 3 0 0 1 17 15h-4l1 4a2 2 0 0 1-3.6 1.5L7 14"/></>,trash:<><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13"/></>
  };
...
export default function ChatPage(){
  ...
  async function send(){
    ...
    try{
      ...
      const d=await r.json();if(!r.ok)throw Error(d.message||'تعذر الوصول إلى KITE AI.');
  ...
  if(!ready)return <div className="center-screen"><div className="brand-loader"><img src="/api/status?icon=1" alt=""/><b>KITE AI</b></div></div>;
  if(status?.settings?.public_chat_enabled===false)return <div className="center-screen"><div className="maintenance-card"><img src="/api/status?icon=1" alt="KITE AI"/><h1>KITE AI</h1><p>النظام في وضع صيانة مؤقتًا. عد لاحقًا.</p></div></div>;
  ...
  return <div className="chat-app public-app" dir="rtl">
    ...
    <aside className={`chat-sidebar ${mobileOpen?'open':''}`}>
      <div className="sidebar-top"><div className="brand-lockup compact"><img className="brand-icon" src="/api/status?icon=1" alt="KITE AI"/><div><strong>KITE AI</strong><span>A SMARTER TOMORROW</span></div></div><button className="icon-btn" onClick={newChat} title="محادثة جديدة"><Icon name="plus"/></button></div>
      ...
      <div className="conversation-list">{chats.length?chats.map(c=><div key={c.id} className={`conversation-wrap ${activeId===c.id?'active':''}`}><button className="conversation-item" onClick={()=>openChat(c.id)}><Icon name="chat" size={16}/><span>{c.title||'محادثة'}</span></button><button className="delete-chat" onClick={()=>removeChat(c.id)} aria-label="حذف"><Icon name="trash" size={15}/></button></div>):<div className="empty-side">ابدأ أول محادثة مع KITE AI</div>}</div>
      <div className="sidebar-footer">{status?.settings?.install_enabled!==false&&!standalone&&<button className="install-side" onClick={install}><Icon name="install" size={17}/><span>تثبيت KITE AI</span></button>}<div className="privacy-mini">المحادثات مجهولة الهوية وقد تُستخدم لتحسين KITE AI. لا تشارك معلومات حساسة.</div></div>
    </aside>

    <main className="chat-main">
      <header className="chat-header"><button className="icon-btn mobile-menu" onClick={()=>setMobileOpen(true)}><Icon name="menu"/></button><div className="model-title"><img className="header-icon" src="/api/status?icon=1" alt=""/><div><strong>KITE AI</strong><small>Learn · Create · Evolve</small></div><span className={`status-dot ${status?.openrouter_configured||status?.self_hosted_configured?'online':'offline'}`}/></div>{status?.settings?.install_enabled!==false&&!standalone&&<button className="install-pill" onClick={install}><Icon name="install" size={16}/><span>تثبيت</span></button>}</header>
      {notice&&<div className="inline-notice">{notice}</div>}
      <section className={`message-stage ${messages.length?'has-messages':'empty-stage'}`}>
        {!messages.length&&<div className="welcome-state"><img className="welcome-logo" src="/api/status?icon=1" alt="KITE AI"/><h1>كيف أستطيع مساعدتك؟</h1><p>محادثة مباشرة مع KITE AI. لا تحتاج إلى حساب.</p><div className="prompt-grid"><button onClick={()=>setInput('حلّل لي هذه الفكرة بعمق وحدد نقاط القوة والضعف.')}>حلّل فكرة بعمق</button><button onClick={()=>setInput('قارن بين أفضل الحلول لهذه المشكلة واختر الأنسب مع السبب.')}>قارن واختر الأفضل</button><button onClick={()=>setInput('ضع لي خطة عملية واضحة لتحقيق هذا الهدف بأقل تكلفة.')}>ابنِ خطة عملية</button></div></div>}
        {messages.map((m,i)=><article key={m.id||i} className={`message-row ${m.role}`}><div className="message-avatar">{m.role==='assistant'?<img src="/api/status?icon=1" alt=""/>:'أ'}</div><div className="message-body"><div className="message-name">{m.role==='assistant'?'KITE AI':'أنت'}</div><div className="message-content">{m.content}</div>{m.role==='assistant'&&m.chatLogId&&<div className="message-tools"><button className={m.feedback==='good'?'selected':''} onClick={()=>rate(m,'good')} title="إجابة جيدة"><Icon name="thumbUp" size={16}/></button><button className={m.feedback==='bad'?'selected':''} onClick={()=>rate(m,'bad')} title="إجابة ضعيفة"><Icon name="thumbDown" size={16}/></button></div>}</div></article>)}
        {busy&&<article className="message-row assistant"><div className="message-avatar"><img src="/api/status?icon=1" alt=""/></div><div className="message-body"><div className="message-name">KITE AI</div><div className="typing"><i/><i/><i/></div></div></article>}
        <div ref={endRef}/>
      </section>
      <footer className="composer-wrap"><div className="composer-box"><textarea rows="1" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} placeholder="اسأل KITE AI..."/><div className="composer-actions">{status?.settings?.public_web_search_enabled&&<button className={`tool-toggle ${webSearch?'active':''}`} onClick={()=>setWebSearch(v=>!v)} title="بحث ويب"><Icon name="globe" size={17}/><span>بحث الويب</span></button>}<div className="composer-spacer"/><button className="send-btn" onClick={send} disabled={busy||!input.trim()}><Icon name="send" size={17}/></button></div></div><div className="composer-note">قد تُستخدم المحادثات المجهولة لتحسين KITE AI. لا ترسل أسرارًا أو بيانات شخصية حساسة.</div></footer>
    </main>
  </div>
}
