...
export async function POST(req){
  try{
    ...
    const runtimeResult=await sb.rpc('get_kite_runtime_config');
    ...
    if(!chat.ok||!payload?.text){
      return NextResponse.json({message:payload?.message||'تعذر تشغيل KITE AI على الاختبار.',blocker:'inference_failed'},{status:chat.status||502});
    }
    ...
    const insert=await sb.from('benchmark_results').insert({
      ...
      model_label:'KITE AI public runtime',
      answer:payload.text,
      score,
