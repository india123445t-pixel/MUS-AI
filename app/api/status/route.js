...
function serviceWorkerResponse(){
  const js=`const C='mus-ai-shell-v4';self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil(caches.open(C).then(c=>c.addAll(['/','/icon.svg'])))});self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(xs=>Promise.all(xs.filter(x=>x!==C).map(x=>caches.delete(x)))).then(()=>self.clients.claim()))});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin||u.pathname.startsWith('/api/chat'))return;if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).then(r=>{const c=r.clone();caches.open(C).then(x=>x.put('/',c));return r}).catch(()=>caches.match('/')));return}e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request)))})`;
  return new Response(js,{headers:{'Content-Type':'application/javascript; charset=utf-8','Service-Worker-Allowed':'/','Cache-Control':'no-cache'}});
}

export async function GET(req){
  const requestUrl=new globalThis.URL(req.url);
  if(requestUrl.searchParams.get('icon')==='1')return NextResponse.redirect(new globalThis.URL('/icon.svg',req.url),307);
  if(requestUrl.searchParams.get('manifest')==='1')return manifestResponse();
  if(requestUrl.searchParams.get('sw')==='1')return serviceWorkerResponse();
  ...
  return NextResponse.json({openrouter_configured:!!process.env.OPENROUTER_API_KEY,self_hosted_configured:!!(process.env.MUS_MODEL_URL||process.env.LOCAL_MODEL_URL),settings});
}
