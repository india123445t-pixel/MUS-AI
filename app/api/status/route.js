...
export async function GET(req){
  ...
  if(search.get('benchmark')==='1'){
    ...
    }catch(e){return NextResponse.json({message:e?.message||'تعذر تشغيل اختبار KITE AI.'},{status:500})}
  }

  if(search.get('intelligence')==='1'){
    if(!client)return NextResponse.json({message:'قاعدة البيانات غير متاحة.'},{status:503});
    try{
      const r=await client.rpc('get_kite_intelligence_snapshot');
  ...
  try{if(client){const r=await client.rpc('get_kite_runtime_config');if(r.data)settings={...defaultSettings,...r.data}}}catch{}
  const providers={
