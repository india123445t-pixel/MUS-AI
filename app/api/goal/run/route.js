...
export async function POST(req){
 try{
  ...
  const cfgRes=await sb.rpc('get_mus_runtime_config');if(cfgRes.error)throw cfgRes.error;const cfg=cfgRes.data||{};
  ...
  if(!r.ok||!d?.text)return NextResponse.json({message:d?.message||'تعذر تشغيل MUS AI عبر جميع مزودي inference المجانيين المتاحين.',blocker:'free_inference_failed'},{status:r.status||503});
 ...
 }catch(e){return NextResponse.json({message:e?.message||'فشلت دورة MUS AI.'},{status:500})}
}
