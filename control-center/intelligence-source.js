...
const names={reasoning:'الاستدلال',math:'الرياضيات',science:'العلوم',coding:'البرمجة',language:'اللغة',research:'البحث',planning:'التخطيط',knowledge:'المعرفة',general:'عام'};
...
export default function IntelligenceLab(){
  ...
  async function save(){if(!settings)return;setBusy(true);setMsg('');const patch={intelligence_router_enabled:!!settings.intelligence_router_enabled,verification_enabled:!!settings.verification_enabled,deep_reasoning_enabled:!!settings.deep_reasoning_enabled,max_model_calls_per_request:Number(settings.max_model_calls_per_request||3),learning_gate_min_confidence:Number(settings.learning_gate_min_confidence||0.72)};const {error}=await sb.from('control_settings').update(patch).eq('owner_id',settings.owner_id);setBusy(false);setMsg(error?error.message:'تم حفظ إعدادات Intelligence Kernel.');if(!error)load()}
  async function runBenchmark(){if(!session)return;setBusy(true);setMsg('');try{const r=await fetch('/api/goal/run',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:'{}'}),d=await r.json();if(!r.ok)throw Error(d.message||'فشل الاختبار');setMsg(`اختبار ${names[d.domain]||d.domain}: ${d.score}/100${d.benchmark?' — '+d.benchmark.suite:''}`);await load()}catch(e){setMsg(e.message)}finally{setBusy(false)}}

  if(!URL||!KEY)return <div className="center-screen"><div className="error-box">إعداد Supabase غير مكتمل.</div></div>;
  if(!ready)return <div className="center-screen"><div className="brand-loader"><img src="/icon.svg" alt=""/><b>MUS AI</b></div></div>;
  if(!session)return <div className="admin-login-shell"><form className="admin-login-card" onSubmit={login}><img src="/icon.svg" alt="MUS AI"/><span className="eyebrow">INTELLIGENCE LAB</span><h1>MUS AI</h1><p>قياس وتحسين الذكاء الموثق.</p><label>البريد الإلكتروني<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label>كلمة المرور<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required/></label><button className="primary-btn" type="submit">دخول</button>{authMsg&&<div className="auth-msg">{authMsg}</div>}</form></div>;

  const verified=logs.filter(x=>x.verification?.verdict==='pass').length,uncertain=logs.filter(x=>['uncertain','unresolved'].includes(x.verification?.verdict)).length;
  ...
  <section className="admin-panel wide" style={{marginTop:12}}><div className="panel-head"><div><span className="eyebrow">REQUEST TRACES</span><h3>آخر قرارات الذكاء</h3><p>هذا هو الدليل الذي سنستخدمه لتحسين الـRouter والـVerifier.</p></div></div><div className="table-list">{logs.length?logs.slice(0,40).map(x=><div className="table-row" key={x.id}><div><b>{names[x.task_contract?.domain]||x.task_contract?.domain||'عام'} · {x.difficulty||'—'}</b><span>{x.route_decision?.mode||'standard'} · {x.route_decision?.web_search?'web':'no-web'} · {x.learning_eligible?'learning candidate':'no learning'}</span></div><span>{x.verification?.verdict||'—'} {x.verification?.confidence!=null?pct(x.verification.confidence):''}</span><strong>{x.model_calls||1} calls</strong><span>{x.latency_ms?`${x.latency_ms}ms`:'—'}</span></div>):<div className="empty-panel">لا توجد traces جديدة بعد. استخدم التطبيق وسيبدأ القياس تلقائيًا.</div>}</div></section>
  </main>
}
