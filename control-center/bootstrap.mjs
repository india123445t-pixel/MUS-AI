...
const ADMIN_RAW='https://raw.githubusercontent.com/india123445t-pixel/KITE-AI/9d286d87dd0ae1309eea3100dedb9f8b1085135a/app/admin/page.js';
...
page=page
  ...
  .replace('href="/" target="_blank">فتح تطبيق KITE AI ↗</a>','href="/intelligence">Intelligence Lab</a><a href="https://ibn-ai-opal.vercel.app/" target="_blank">فتح تطبيق KITE AI ↗</a>');
...
export const metadata={title:'KITE AI Control Center',description:'Private KITE AI administration and model control center',robots:{index:false,follow:false}};
...
const statusRoute=`const BASE=process.env.KITE_PUBLIC_APP_URL||'https://ibn-ai-opal.vercel.app';
export async function GET(){
  try{const r=await fetch(BASE+'/api/status',{cache:'no-store'});const body=await r.text();return new Response(body,{status:r.status,headers:{'content-type':r.headers.get('content-type')||'application/json','cache-control':'no-store'}})}catch{return Response.json({message:'تعذر الاتصال بتطبيق KITE AI.'},{status:502})}
}`;
...
// runtime policy before it calls the public KITE AI inference router.
...
console.log('KITE AI Control Center source synchronized.');
