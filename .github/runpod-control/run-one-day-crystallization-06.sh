...
test "$(git rev-parse "$SOURCE_HEAD")" = "$SOURCE_HEAD"
...
scp -q -i /tmp/auth06_key -P "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null   /tmp/crystal-source.tgz /tmp/public-pack.json /tmp/w02.py /tmp/recovered-materialized-probe.json   .github/runpod-control/auth06-crystallization-remote.sh root@"$host":/tmp/
ssh -i /tmp/auth06_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host"   'mkdir -p /workspace; rm -f /workspace/auth06.rc; nohup setsid bash /tmp/auth06-crystallization-remote.sh >/workspace/auth06-supervisor.log 2>&1 < /dev/null &'
