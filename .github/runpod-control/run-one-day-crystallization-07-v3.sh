...
RESULT=".github/runpod-control/one-day-crystallization-run-result-07.json"
...
AUTH_SHA="7cdc2be72a407fed6e25333e4689c8dd26cda9f3d1a5e606f002fe4f1658c379"
...
cleanup() {
  ...
  if [ -n "$pod" ] && [ "$cleaned" != 1 ]; then
    echo "AQLEVON_AUTH07_EXIT_CLEANUP pod=$pod status=$status"
...
assert sha==a["authorization_sha256"]=="7cdc2be72a407fed6e25333e4689c8dd26cda9f3d1a5e606f002fe4f1658c379"
...
print("AQLEVON_AUTH07_DRIVER_CONTRACT_PASS")
PY
...
tar --owner=0 --group=0 --numeric-owner -czf /tmp/crystal-source.tgz -C /tmp/crystal-source .
sha256sum /tmp/crystal-source.tgz /tmp/public-pack.json /tmp/w02.py /tmp/recovered-materialized-probe.json > /tmp/auth07-input-sha256.txt
echo AQLEVON_AUTH07_EXACT_SOURCE_STAGED_BEFORE_BILLING
...
git add .github/runpod-control/one-day-crystallization-live-selection-07.json
...
runpodctl pod create --name AQLEVON-ONE-DAY-CRYSTALLIZATION-07   --image "ghcr.io/india123445t-pixel/mus-ai@$IMAGE_DIGEST"   --gpu-id "$(cat /tmp/selected_gpu)" --gpu-count 1 --cloud-type SECURE   --data-center-ids "$(cat /tmp/selected_dc)" --container-disk-in-gb 40   --volume-in-gb 40 --volume-mount-path /workspace --ports 22/tcp --env "$env_json" --output json > /tmp/create06.json
...
out={"authorization_id":"P4-ONE-DAY-CRYSTALLIZATION-20260925-07",
     "authorization_sha256":"7cdc2be72a407fed6e25333e4689c8dd26cda9f3d1a5e606f002fe4f1658c379",
     "source_head":"f3a2ca4c894e81cf9dffa1dd9935a3774b854cdb",
     "image_digest":"sha256:ad4f48dd206b317e09d8fe1a834e57e79c444f9f581ebd45179c4072cb0d66ec",
...
PY
scp -q -i /tmp/auth07_key -P "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null /tmp/hw06.py root@"$host":/tmp/hw06.py
ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host" python3 /tmp/hw06.py

scp -q -i /tmp/auth07_key -P "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null   /tmp/crystal-source.tgz /tmp/public-pack.json /tmp/w02.py /tmp/recovered-materialized-probe.json   .github/runpod-control/auth07-crystallization-remote-v3.sh root@"$host":/tmp/
ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host"   'mkdir -p /workspace; rm -f /workspace/auth07.rc; nohup setsid bash /tmp/auth07-crystallization-remote-v3.sh >/workspace/auth07-supervisor.log 2>&1 < /dev/null &'
echo AQLEVON_AUTH07_REMOTE_START

rc=255
for i in $(seq 1 240); do
  if ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=8 root@"$host" 'test -s /workspace/auth07.rc' >/dev/null 2>&1; then
    rc="$(ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host" 'cat /workspace/auth07.rc' | tail -n1)"
    break
  fi
  elapsed=$(( $(date +%s) - $(cat /tmp/pod_created_epoch) ))
...
ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host"   'cd /workspace && tar -czf /tmp/auth07-evidence.tgz auth07 auth07-supervisor.log auth07.rc 2>/dev/null' || true
ssh -i /tmp/auth07_key -p "$port" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@"$host"   'cat /workspace/auth07/train.log' >/tmp/auth07-train.log 2>/dev/null || true
...
if Path("/tmp/auth07-evidence.tgz").exists():
    try:
        t=tarfile.open("/tmp/auth07-evidence.tgz","r:gz"); root="auth07/candidate/"
