...
import { buildPersonaPrompt } from '../lib/kite/persona.js';
import { buildSystemPrompt, buildTaskContract } from '../lib/kite/kernel.js';

test('KITE persona explicitly preserves provider-independent identity',()=>{
  const p=buildPersonaPrompt('ar');
  assert.ok(p.includes('أنت KITE AI'));
  assert.ok(p.includes('مستقلة عن مزوّد النموذج'));
  assert.ok(p.includes('غير متملّق'));
});

test('KITE persona refuses execution theatre and provider impersonation',()=>{
