...
async function mockEndpoint({chatStatus=200}={}){
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(req.url==='/v1/chat/completions'){
      ...
      res.end(JSON.stringify({id:'mock-1',object:'chat.completion',model:TARGET,choices:[{index:0,message:{role:'assistant',content:'AQLEVON sovereign mock response'}}]}));
      return;
    }
...
test('mock OpenAI-compatible endpoint proves self_hosted_only request/response without external inference',async()=>{
  ...
  try{
    ...
    assert.equal(result.provider,'aqlevon-engine');
    assert.equal(result.model,TARGET);
    assert.equal(result.text,'AQLEVON sovereign mock response');
...
test('self_hosted_only failure does not fall back to OpenRouter/Gemini/other external providers',async()=>{
  ...
  try{
    ...
    assert.equal(result.diagnostics[0].provider,'aqlevon-engine');
    assert.deepEqual(seen,[`${mock.base}/v1/chat/completions`]);
  }finally{globalThis.fetch=originalFetch;restoreEnv(saved);mock.server.close();await once(mock.server,'close')}
});
...
test('self-hosted health and descriptor are secret-safe',async()=>{
  const mock=await mockEndpoint();
  const saved=saveEnv(['AQLEVON_MODEL_URL','AQLEVON_MODEL_NAME','AQLEVON_MODEL_KEY']);
  try{
    process.env.AQLEVON_MODEL_URL=`${mock.base}/v1/chat/completions`;process.env.AQLEVON_MODEL_NAME=TARGET;process.env.AQLEVON_MODEL_KEY='super-secret-health-key';
    ...
    assert.deepEqual(health,{ok:true,configured:true,provider:'aqlevon-engine',model:TARGET,status:200,error_class:null});
