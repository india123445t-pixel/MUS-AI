...
import {checkSelfHostedHealth,generateModelResponse,getSelfHostedRuntimeDescriptor,resolveSelfHostedConfig} from '../lib/mus/providers.js';
...
async function mockEndpoint({chatStatus=200}={}){
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(req.url==='/v1/chat/completions'){
      ...
      res.end(JSON.stringify({id:'mock-1',object:'chat.completion',model:TARGET,choices:[{index:0,message:{role:'assistant',content:'MUS sovereign mock response'}}]}));
      return;
    }
...
test('Qwen sovereign target is selectable through settings or env without provider hard-code',()=>{
  const saved=saveEnv(['MUS_MODEL_NAME','MUS_MODEL_URL']);
  try{
    process.env.MUS_MODEL_NAME=TARGET;process.env.MUS_MODEL_URL='http://127.0.0.1:9999/v1';
...
test('mock OpenAI-compatible endpoint proves self_hosted_only request/response without external inference',async()=>{
  ...
  const keys=['MUS_MODEL_URL','MUS_MODEL_NAME','MUS_MODEL_KEY','OPENROUTER_API_KEY','GROQ_API_KEY','GEMINI_API_KEY','MISTRAL_API_KEY','CEREBRAS_API_KEY','HF_TOKEN'];
  const saved=saveEnv(keys);const originalFetch=globalThis.fetch;const seen=[];
  try{
    process.env.MUS_MODEL_URL=`${mock.base}/v1`;
    process.env.MUS_MODEL_NAME=TARGET;
    process.env.MUS_MODEL_KEY='local-test-key';
    ...
    assert.equal(result.provider,'mus-engine');
    assert.equal(result.model,TARGET);
    assert.equal(result.text,'MUS sovereign mock response');
...
test('self_hosted_only failure does not fall back to OpenRouter/Gemini/other external providers',async()=>{
  ...
  const keys=['MUS_MODEL_URL','MUS_MODEL_NAME','OPENROUTER_API_KEY','GROQ_API_KEY','GEMINI_API_KEY','MISTRAL_API_KEY','CEREBRAS_API_KEY','HF_TOKEN'];
  const saved=saveEnv(keys);const originalFetch=globalThis.fetch;const seen=[];
  try{
    process.env.MUS_MODEL_URL=`${mock.base}/v1/chat/completions`;process.env.MUS_MODEL_NAME=TARGET;
    ...
    assert.equal(result.diagnostics[0].provider,'mus-engine');
    assert.deepEqual(seen,[`${mock.base}/v1/chat/completions`]);
  }finally{globalThis.fetch=originalFetch;restoreEnv(saved);mock.server.close();await once(mock.server,'close')}
});
...
test('self-hosted health and descriptor are secret-safe',async()=>{
  const mock=await mockEndpoint();
  const saved=saveEnv(['MUS_MODEL_URL','MUS_MODEL_NAME','MUS_MODEL_KEY']);
  try{
    process.env.MUS_MODEL_URL=`${mock.base}/v1/chat/completions`;process.env.MUS_MODEL_NAME=TARGET;process.env.MUS_MODEL_KEY='super-secret-health-key';
    ...
    assert.deepEqual(health,{ok:true,configured:true,provider:'mus-engine',model:TARGET,status:200,error_class:null});
