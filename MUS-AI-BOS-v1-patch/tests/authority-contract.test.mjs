...
import { createTrustedRegistry, readQualification } from '../lib/mus/tool-registry.js';
import { canonicalResource, createActionIntent, issuePermit, preDispatchCheck } from '../lib/mus/authority.js';
import { transitionActionAttempt, transitionTask, transitionVerification } from '../lib/mus/state-machines.js';
import { createActionReceipt, receiptProvesWorldState } from '../lib/mus/receipts.js';
import { buildModelContext, criticalContextManifest, validateCompaction } from '../lib/mus/context.js';
...
const resource={provider:'github',tenant:'owner',project:'MUS-AI',environment:'production',resource_id:'repo-123'};
...
test('active DurableConstraint denial prevents Permit issuance',()=>{
  ...
  const constraints=[{constraint_id:'dc1',version:1,status:'ACTIVE',scope_ref:'MUS-AI',applicability:{tenant:'owner',project:'MUS-AI',capabilities:['WRITE']},rule_hash:'h',rule_spec:{effect:'DENY'}}];
...
test('changing applicable DurableConstraint set invalidates unconsumed Permit at dispatch',()=>{
  const intent=createActionIntent({task_id:'t1',contract_id:'c1',semantic_action:'update-file',resource,parameters:{path:'x'},capabilities:['WRITE']});
  const permit=issuePermit({principal,taskContract:contract,intent,toolSpec:tool(),constraints:[]});
  const newConstraint={constraint_id:'dc2',version:1,status:'ACTIVE',scope_ref:'MUS-AI',applicability:{project:'MUS-AI'},rule_hash:'h2',rule_spec:{effect:'REQUIRE'}};
